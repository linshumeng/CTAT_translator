...
USE_DEEPL = False # Translate via commercial deepl API
...
TRANSLATION_DICT = PROJECT_DIR+'translations.json'
...
LANGUAGE_STRING = 'espanol' # generic string to identify language

TOKEN_DIR = PROJECT_DIR+'token.txt'
GLOSSARY_DIR = PROJECT_DIR+'glossary.csv'
...
def save_translations(d_out, f=TRANSLATION_DICT):
    with open(f, 'w', encoding='utf8') as handle:
...
if not SKIP_HTML:
    for f_str in tqdm(fs_html):
        with open(f_str, 'r') as infile:
            ...
            for found in soup.findAll(elements, attrs={'class': 'site-content'}):
                print(found.string)
                if found.string in repeated_translations:
                    ...
                    try:
                        if USE_DEEPL:
                            ...
                            # print(found.string)
                            translation = tss.google(found.string, from_language='en', to_language=TARGET_LANG)
...
def make_var(phrase, signature='_', keep_n_words=4):
    ...
    print('phrase', phrase)
    print('the_clean_phrase', the_clean_phrase)
    var_phrase_map[v] = the_clean_phrase
    # print(var_phrase_map)
    ...
    if the_clean_phrase in repeated_translations:
        ...
        try:
            if USE_DEEPL:
                ...
                translation = tss.google(the_clean_phrase, from_language='en', to_language=TARGET_LANG)
                # print(translation)
...
def process_file(infile, outfile_brd, outfile_massprod):
    ...
    # print("var_phrase_map", var_phrase_map)
    # print("var_translation_map", var_translation_map)
    # print(var_phrase_map.keys())
    signature = infile.split('/')[-1].split('.brd')[0]
    with open(outfile_massprod, "w", encoding='utf-8') as f:
